...
def ascon_process_plaintext(S, b, rate, plaintext):
    ...
    for block in range(0, len(p_padded) - rate, rate):
        if rate == 8:
            ...
            if debugpermutation : print("-- Texte chiffré C" + str(int(block/rate)+1) + " = 0x " + bytes_to_hex(int_to_bytes(S[0], 8)))
        elif rate == 16:
            S[0] ^= bytes_to_int(p_padded[block:block+8])
    ...
    if rate == 8:
        ...
        if debugpermutation : print("-- Texte chiffré C" + str(int(block/rate)+1) + " = 0x " + bytes_to_hex(int_to_bytes(S[0], 8)[:p_lastlen]))
    elif rate == 16:
        S[0] ^= bytes_to_int(p_padded[block:block+8])
        S[1] ^= bytes_to_int(p_padded[block+8:block+16])
...
def ascon_permutation(S, rounds=1):
    ...
    for r in range(12-rounds, 12):
        ...
        if (debugtransformation or debugFull): 
            if debugpermutation : print("-- Permutation (r=" + "{:02d}".format(r) + ")")
            if debugpermutation : myprintstate(S, "Addition constante : ")
        # --- substitution layer ---
        S[0] ^= S[4]
        S[4] ^= S[3]
        S[2] ^= S[1]
